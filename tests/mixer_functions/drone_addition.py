...
# Load the iamges
overlay_original    = cv2.imread('images/drone_overlay.png')
...
overlay             = cv2.bitwise_not(overlay, mask=mask)
overlay             = cv2.bitwise_not(overlay)

# Copy the background where the mask is not pure white
output = copy.deepcopy(background)
output[np.where(overlay != 255)] = overlay[np.where(overlay != 255)]
